...
- **PostgreSQL** schema owned by Flyway (`V1__initial_schema.sql` baseline, consolidated from the original V1-V5 while the app was still greenfield in testing + `V2__nfc_fault_reports.sql`).
...
| `ui/`, `util/`, `mapper/` | Yes |
| `src/main/resources/db/migration/V1__*.sql` | Yes — baseline (consolidated; includes `api_sessions`, ops/monitoring permissions, log-sheet cancel — the original V2-V5 folded in while still greenfield) |
| `src/main/resources/db/migration/V2__nfc_fault_reports.sql` | Yes — `nfc_fault_reports` table, `log_sheet_entries.entry_source`/`filled_by_user_id`, related permissions |
...
| `db/migration/V2__nfc_fault_reports.sql` | `nfc_fault_reports` + per-entry authorship columns + related permissions |
