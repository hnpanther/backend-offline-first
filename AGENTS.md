...
51. **An asset's `status` column is driven by the log sheet, and every change is journalled so it can be undone exactly.** When a sheet completes, `AssetStatusService.applyFromCompletedSheet` copies the reading of any class field keyed `status` — matched **case-insensitively**, so `Status`/`STATUS` all count — onto `asset_entries.status`, and writes an `asset_status_history` row carrying *both* the old and new value. Recording `oldStatus` is the whole design: a reversal restores that exact value instead of re-deriving "what it was before" from earlier history, which would be slower and wrong the moment anything else touched the column in between. The hooks are `LogSheetService` (mobile batch, `completeFromWeb`, `finalizeDraftOnExpiry`) for apply, and `LogSheetAssignmentService` for undo — `voidSubmitted` and `reopenSubmittedWithExtend` call `revertForSheet`, while `restoreVoided` calls `applyFromCompletedSheet` again (it replays the readings rather than un-reverting, so the result is correct even if the sheet's data changed). `cancel` needs no hook: it is guarded to `OPEN_FOR_OWNERSHIP_CHANGE` and can never see a SUBMITTED sheet. **Apply must run after entries are persisted**, since it reads them back. Three rules protect live data and are each pinned by a test: a blank reading is ignored (blank means "not recorded", not "has no state"); a no-op change writes no history row (a row saying IN_SERVICE → IN_SERVICE is noise); and **a reversal only restores when the asset still holds the value this sheet set** — if a later sheet or a manual edit moved it on, the reversal is skipped and logged rather than rolling an old completion back over a newer truth (`neverRollsBackOverAValueSomethingElseSetLater`). Performance is per **sheet**, not per asset: one query for entries, one `findAllById` for assets, one indexed query for the active history (`idx_asset_status_history_active`, partial on `reverted_at IS NULL AND change_type = 'APPLIED'`), then batch saves — a 50-asset sheet costs a constant handful of statements. A `multiselect` status arrives as a **collection** and is joined with `", "`; `String.valueOf` on a list would store `[on, IDLE]`, brackets and all. Values longer than the 30-char column are truncated with a warning rather than failing the completion, because losing an operator's whole round over one over-long value is the worse outcome.
