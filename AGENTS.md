...
68. **Bulk imports produce one async audit task per saved row, and the audit queue is bounded.** `RepositoryAuditAspect` advises every `CrudRepository.save`, so importing N rows enqueues N tasks onto `auditExecutor` (2–4 threads, `queueCapacity` 256, default AbortPolicy). A tight import loop is an unbounded producer against that queue: when it fills, `save()` throws `TaskRejectedException` **into the import thread**, whose message — "ExecutorService in active state did not accept task" — names an executor that has nothing to do with importing and mentions no row. Verified against the live data: a 1,143-row `main-functions` import left exactly 1,143 `audit_log` CREATE rows. Anything that writes thousands of entities in a loop has to reckon with this.
---
