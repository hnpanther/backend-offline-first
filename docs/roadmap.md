...
# 2. Integration API for third-party systems

*Raised as: an authenticated, key-based API for external systems to read completed log sheets.*

## What exists to build on

| Fact | Where |
|---|---|
| Two independent filter chains already: stateless JWT for `/api/**` (`@Order(1)`), session + form login for the panel (`@Order(2)`) | `WebSecurityConfig` |
| `JwtAuthenticationFilter` is the pattern to copy: a valid signature is **not** enough — the `jti` must match a live `api_sessions` row, which is what makes revocation take effect on the next request | `security/JwtAuthenticationFilter` |
| Every endpoint is gated by a permission whose **code is the route**, seeded in a migration. No permission row → nobody can reach it, not even an admin | [security.md](security.md#309) |
| Access is three layers — endpoint, scope, object — and holding the endpoint permission proves nothing about the other two | [security.md](security.md#1-the-three-layers) |
| `audit_log` exists for **changes**; retention is driven by `audit.retention.days` in `app_settings` | [schema.md](schema.md) |
| The codebase uses **no `Pageable` anywhere** — introducing Spring Data paging would be a new pattern | — |
| An expiry-finalised draft is also `SUBMITTED` | [jobs.md](jobs.md#log-sheet-expiry) |

## Shape

**A third filter chain**, `securityMatcher("/integration/**")` at `@Order(0)`, ahead of the API
chain: no form login, no JWT, no session — an `X-API-Key` header and nothing else. This is what
"third-party endpoints must be separate from normal user authentication" means concretely.

### `api_keys`

```
id, client_name, key_id (unique), secret_hash, prefix,
active, created_at, created_by_user_id,
expires_at, revoked_at, revoked_by, revoke_reason,
scope_unit_ids (nullable JSONB), last_used_at
```

Three decisions worth stating:

- **The key is shown once.** Format `lsk_<key_id>_<secret>`; only a hash is stored. `key_id` is
  the lookup column, so verification is one indexed read plus one hash comparison rather than a
  scan that hashes every row.
- **Revoke, never delete.** The row survives so past audit entries stay attributable, and
  disabling one integration cannot affect another.
- **Optional unit scope.** When set, the existing `visibleUnitIds()` machinery applies. Without
  it every key sees the whole plant, which contradicts "sensitive internal data should not be
  exposed".

### Endpoints

```
GET /integration/v1/log-sheets?completedFrom=&completedTo=&page=&size=
GET /integration/v1/log-sheets/{id}
```

- Only `status = 'SUBMITTED'` with `completed_at` inside the range. **Decide explicitly** what
  to do with drafts the expiry job auto-finalised — they are genuinely `SUBMITTED`, so they
  should probably be returned, flagged as auto-finalised. `VOIDED` must never be returned.
- **Filter in the repository, not the service or the DTO.** A filter applied higher up is a hole
  the first person to reuse the method will open — the same reasoning as
  [security.md](security.md#1-the-three-layers) on service-layer checks.
- **Dedicated DTOs.** Reusing the mobile DTOs means every internal field added to them later
  leaks silently. Explicitly out: `sync_status`, `client_action_id`,
  `field_definitions_snapshot`, internal user ids, and anything the integration did not ask for.
- Pagination: an explicit `{items, page, size, total}` with a hard maximum, matching the
  existing style rather than introducing `Pageable` for two endpoints.
- Attachments: metadata plus a link in the detail response; a separate endpoint behind the same
  key if the bytes are actually needed.

### Auditing

A dedicated lightweight table (`key_id`, endpoint, parameters, status, row count, timestamp) is
better than `audit_log`, which exists for changes and would be drowned by a polling integration.
Retire rows with the existing retention setting.

### Admin UI

An "integration keys" page: create (value shown once), list with last-used, disable and revoke
with a reason. Three new permissions in a **new migration** — `V1` is a closed baseline.

## Build order

1. Migration (table + permission rows)
2. Filter chain and key-verification service
3. Admin page
4. The two endpoints with their own DTOs
5. Usage auditing
6. Integration tests: invalid / revoked / expired / disabled key, date range, pagination,
   an unfinished sheet must not appear, no internal field leaks
7. Docs — `security.md` (a fourth authentication surface), `schema.md`, `log-sheets.md`, README

## Two risks worth knowing before starting

- **This is the first request path in the system with no user.** Anything that assumes a current
  principal — `SecurityUtils`, the logging aspect, the MDC filter — has to tolerate its absence.
- **Read volume is the opposite of the rest of the app.** An integration polling every minute
  behaves nothing like a tablet syncing every thirty seconds; index the `completed_at` range
  query for it deliberately rather than assuming the existing indexes cover it.
