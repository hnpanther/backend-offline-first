...
| V3 | `V3__capabilities_user_org_fields_and_policy_settings.sql` | Three things, kept in one file because production has never run any of them (it is still on V2). **(a)** Eleven `CAP:*` rows in `permissions` (`category = 'capability'`, null method/path) plus the `role_permissions` grants that reproduce the previous role-code behaviour exactly — this is what makes a duplicated role behave like its original, see [security.md](security.md#3-capabilities--access-that-is-not-about-an-endpoint). **(b)** `users.org_unit` and `users.org_position`: optional free-text personnel attributes (150 chars, nullable, not unique), deliberately unrelated to `operational_units`. **(c)** Seeds `attachments.image_annotation_enabled`, `nfc.strict_serial_match` and `nfc.manual_entry_enabled` (all `true`) into `app_settings`, `ON CONFLICT DO NOTHING` so an installation that already chose a value keeps it. **Applied — the next change is V4.** Renamed once already: it started as `V3__role_capabilities.sql` and outgrew that name. Renaming an applied migration needs the same manual repair as merging one — Flyway validates `description` as well as the checksum — so `flyway_schema_history.description` and `script` were realigned by hand on development databases. The checksum is over the file's *contents*, so a rename alone leaves it untouched. |
...
`status` ∈ `PENDING`, `ASSIGNED`, `IN_PROGRESS`, `SUBMITTED`, `VOIDED`, `EXPIRED`, `CANCELLED`.
`origin` ∈ `MANUAL`, `SCHEDULED`. `assignment_type` ∈ `SELF_CLAIMED`, `SUPERVISOR_ASSIGNED`.
