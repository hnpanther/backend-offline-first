...
--   5. audit_log.actor_user_id loses its foreign key, so an audit row can be written after
...
--
-- Measured, not theorised. One full `mvn test` run logs 42 `Audit row lost` warnings, every one
-- of them `violates foreign key constraint "fk_audit_log_actor_user"`, while all 1472 tests stay
-- green — because AuditWriteService catches the failure and warns rather than throwing. That is
-- precisely how it behaves in production: silently, and only in the log.
--
-- WHY THE CONSTRAINT IS DROPPED, AND NOT SET TO `ON DELETE SET NULL`
--
-- SET NULL was the first attempt at this fix and it does not work.
--
-- A referential action fires when the PARENT row is deleted, and it rewrites the child rows that
-- exist AT THAT MOMENT. The rows this problem is about do not exist yet: they arrive afterwards,
-- carrying an actor_user_id that no longer resolves. There is nothing for SET NULL to act on, so
-- the late INSERT violates the constraint exactly as it did under RESTRICT.
--
-- SET NULL repairs only the case where the audit row was already written when the account was
-- removed — which was never the case that was losing data, because a written row makes
-- `hasAppActivity` answer "yes" and the delete is refused before it starts. The one arrangement
-- SET NULL helps is the one the guard already prevents.
--
-- Dropping the constraint is what actually closes it: an audit row can be inserted whether or
-- not its actor still exists, which is the property the trail needs.
--
-- WHY LOSING REFERENTIAL INTEGRITY HERE IS CORRECT AND NOT A COMPROMISE
--
-- A foreign key asserts "this points at a row that exists". `actor_user_id` does not mean that.
-- It means "the account with this id did this, at the time it happened" — a statement about the
-- past, which stays true after the account is gone. That is the same reason `actor_username` is
-- denormalised next to it rather than joined: the trail has to stay readable when the user is
-- deleted or renamed. The id now follows the rule the username already followed.
--
-- Nothing in the application depends on the constraint. `AuditLog.actorUserId` is a plain Long,
-- not a @ManyToOne, so no JPA mapping and no query joins through it; the audit screen renders
-- `actor_username`. Hibernate's `ddl-auto=validate` checks tables, columns and types — not
-- foreign keys — so the boot-time schema check is unaffected.
--
-- The one thing to know: Postgres identity columns do not reuse ids in practice, but `users.id`
-- is GENERATED BY DEFAULT, so an explicit insert could in principle reuse a deleted id and make
-- an old audit row appear to name a different person. Read `actor_username` — it is the
-- authoritative field, and it is what the UI shows.
--
-- WHY NOT DRAIN THE QUEUE BEFORE EACH DELETE
--
-- It would fix step 3 and leave the shape of the problem intact: the trail would still depend on
-- a race being won. And it would couple deleting a user to the health of the audit executor,
-- which is the coupling that already cost this project an import job stuck at RUNNING (see
-- AsyncConfig's CallerRunsPolicy note).
--
-- log_sheet_action_log is left alone, but NOT because it has no foreign key — it has three
-- (fk_lsal_actor_user, fk_lsal_from_user, fk_lsal_to_user, all ON DELETE RESTRICT). It is safe
-- because LogSheetActionLogger writes SYNCHRONOUSLY, inside the caller's transaction. Its rows
-- are therefore always visible to `hasAppActivity` before a delete is allowed, so no write can
-- ever land after its actor is gone. RESTRICT is right there, for the same reason it was wrong
-- here.
--
...
-- IF EXISTS: development databases were realigned onto this file by hand (gotcha #86) and some
-- had already applied the earlier SET NULL form of this section. The constraint carries the same
-- name either way, and dropping it must succeed from both starting points.
ALTER TABLE audit_log
    DROP CONSTRAINT IF EXISTS fk_audit_log_actor_user;

COMMENT ON COLUMN audit_log.actor_user_id IS
    'Who made the change, as a historical fact — deliberately NOT a foreign key. Audit rows are '
    'written asynchronously and can land after the account is deleted; a foreign key makes that '
    'write fail and the row is lost. actor_username is denormalised alongside and is the field '
    'to read: it survives deletion and rename, and it is what the audit screen shows.';
