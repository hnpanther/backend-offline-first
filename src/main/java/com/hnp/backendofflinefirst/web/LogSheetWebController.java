...
public class LogSheetWebController {
    ...
    public String list(@RequestParam(required = false) String status, Model model) {
        ...
        var sheets = logSheetRepository.findAll();
        if (status != null && !status.isBlank()) {
            sheets = sheets.stream().filter(s -> status.equals(s.getStatus())).toList();
        }
        model.addAttribute("logSheets", sheets);
    ...
    public String detail(@PathVariable String id, Model model) {
        model.addAttribute("activePage", "log-sheets");
        logSheetRepository.findById(id).ifPresent(sheet -> {
